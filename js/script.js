...
function setInputFilled(input, isFilled) {
    input.classList.toggle("filled", isFilled)
    const slot = input.closest(".otp-slot")
    if (slot) {
...
function initMatrixRain() {
    ...
    const matrix = "BIRTHDAYFYORBIT10092005LETHUANPHI"
    ...
    function draw() {
        // Semi-transparent deep-space layer for the neon trail effect
        ctx.fillStyle = "rgba(1, 4, 12, 0.075)"
        ...
        for (let i = 0; i < drops.length; i++) {
            ...
            ctx.fillStyle = colorRoll > 0.985 ? ice : colorRoll > 0.95 ? lime : colorRoll > 0.9 ? violet : cyan
            ctx.shadowColor = ctx.fillStyle
            ctx.shadowBlur = colorRoll > 0.95 ? 12 : 4
...
function checkPin() {
    ...
    if (inputPin === correctPin) {
        ...
        errorMessage.innerHTML = '<i class="bi bi-bug"></i> Access denied. Re-enter launch code.'
    ...
    otpInputs.forEach((input, index) => {
        input.addEventListener("input", function (e) {
            ...
            if (this.value.length === 1) {
                setInputFilled(this, true)
                createInputParticles(this)
            } else {
                setInputFilled(this, false)
            }
        ...
        input.addEventListener("paste", (e) => {
            ...
            for (let i = 0; i < pastedData.length && index + i < otpInputs.length; i++) {
                ...
                setInputFilled(targetInput, true)
...
function createInputParticles(input) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        particle.style.cssText = `
            ...
            left: ${centerX}px;
            top: ${centerY}px;
            box-shadow: 0 0 10px rgba(55, 231, 255, 0.82);
        `
...
function createSuccessEffect() {
    ...
    for (let i = 0; i < 20; i++) {
        ...
        particle.style.cssText = `
            ...
            left: ${centerX}px;
            top: ${centerY}px;
            box-shadow: 0 0 14px rgba(55, 231, 255, 0.86);
        `
    ...
    pinCard.style.boxShadow = `
        0 0 60px rgba(55, 231, 255, 0.42),
        0 0 100px rgba(156, 255, 203, 0.18),
        inset 0 1px 0 rgba(238, 248, 255, 0.18)
    `
}
