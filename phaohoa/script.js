...
const selectableShellNames = shellNames.filter(name => name !== 'Crysanthemum');

function randomShellName() {
  return Math.random() < 0.5
    ? 'Crysanthemum'
    : selectableShellNames[(Math.random() * selectableShellNames.length) | 0];
}

function randomFastShell() {
  let shellName = shellNameSelector() === 'Random' ? randomShellName() : shellNameSelector();

  return shellTypes[shellName] || crysanthemumShell;
}
...
function render(speed) {
  ...
  trailsCtx.fillStyle = `rgba(0, 0, 0, ${0.145 * speed})`;
...
class Shell {
  ...
  burst(x, y) {
    ...
    const starFactory = (angle, speedMult) => {
      ...
      // bursts appear visually centered for the majority of the star life.
